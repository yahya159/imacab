...
function ProductItem({ product }) {
  const productImage = product?.attributes?.banner?.data?.attributes?.url || '/default-product-image.jpg';
  ...
  const productPrice = product?.attributes?.price ? `$${product?.attributes?.price.toFixed(2)}` : 'Price not available';
